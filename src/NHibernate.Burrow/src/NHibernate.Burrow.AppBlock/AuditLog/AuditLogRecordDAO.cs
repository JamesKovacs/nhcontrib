...
namespace NHibernate.Burrow.Util.AuditLog
{
    ...
    public class AuditLogRecordDAO : GenericDAO<AuditLogRecord>
    {
        ...
        {
            return
                GetCriteria().Add(Expression.Eq("EntityId", entityId.ToString())).Add(
                    Expression.Eq("EntityType", entityType.Name.ToString())).AddOrder(new Order("Created", false)).List
                    <AuditLogRecord>();
        }
        ...
        public IList<AuditLogRecord> Find(System.Type entityType, object entityId, string action)
        {
            return
                GetCriteria().Add(Expression.Eq("EntityId", entityId.ToString())).Add(
                    Expression.Eq("EntityType", entityType.Name.ToString())).Add(Expression.Eq("Action", action)).List
                    <AuditLogRecord>();
        }
    }
}