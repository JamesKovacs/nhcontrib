...
namespace NHibernate.Burrow.Util.NH.Impl
{
    /// <summary>
    ...
    public abstract class AbstractDetachedQuery : IDetachedQuery, IDetachedQueryImplementor
    {
        ...
        protected readonly Dictionary<string, IEnumerable> namedUntypeListParams =
            new Dictionary<string, IEnumerable>(2);
        ...
        {
            posParams[position] = new TypedValue(type, val, entityMode);
            return this;
        }
        ...
        public IDetachedQuery SetParameter(string name, object val, IType type)
        {
            ...
            namedParams[name] = new TypedValue(type, val, entityMode);
            return this;
        }
        ...
        public IDetachedQuery SetParameterList(string name, IEnumerable vals, IType type)
        {
            ...
            namedListParams[name] = new TypedValue(type, vals,entityMode);
            return this;
        }
        ...
        public IDetachedQuery SetIgnoreUknownNamedParameters(bool ignoredUnknownNamedParameters)
        {
            throw new NotSupportedException(
                "Supported only from NH 2.0: if you are using the NH2.0+ you can use the DetachedQuery class in it.");
        }
        ...
        {
            destination.SetMaxResults(selection.MaxRows).SetFirstResult(selection.FirstRow).SetCacheable(cacheable).
                SetForceCacheRefresh(forceCacheRefresh).SetTimeout(selection.Timeout).SetFlushMode(flushMode);
        ...
        {
            q.SetMaxResults(selection.MaxRows).SetFirstResult(selection.FirstRow).SetCacheable(cacheable).SetCacheMode(cacheMode)
              .SetTimeout(selection.Timeout).SetFlushMode(flushMode);
            if (!string.IsNullOrEmpty(cacheRegion))
            {
