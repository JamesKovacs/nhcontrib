...
namespace NHibernate.Burrow.Test.AuditLogTest {
    ...
    public class AuditLogTest : TestBase {
        ...
        public void AuditTest() {
            ...
            MockEntity mpc = new MockEntity();
            mpc.Save();
            CommitAndClearSession();

            mpc = MockEntityDAO.Instance.FindById(mpc.Id);
            ...
            IList<AuditLogRecord> result = AuditLogRecordDAO.Instance.Find(typeof (MockEntity), mpc.Id);
            ...
            result = AuditLogRecordDAO.Instance.Find(typeof (MockEntity), mpc.Id, Actions.UPDATE);
            ...
            result = AuditLogRecordDAO.Instance.Find(typeof (MockEntity), mpc.Id, Actions.INSERT);
            Assert.AreEqual(1, result.Count);

            result = AuditLogRecordDAO.Instance.Find(typeof (MockEntity), mpc.Id, Actions.DELETE);
            Assert.AreEqual(1, result.Count);
        }
    }
}