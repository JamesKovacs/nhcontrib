...
namespace NHibernate.Caches.SharedCache
{
	...
	public class SharedCacheProvider : ICacheProvider
	{
		...
		{
			_log = LogManager.GetLogger(typeof (SharedCacheProvider));
			SharedCacheConfig[] configs = ConfigurationManager.GetSection("sharedcache") as SharedCacheConfig[];
			if (configs != null)
			{
				foreach (SharedCacheConfig config in configs)
				{
				}
			}
		}
