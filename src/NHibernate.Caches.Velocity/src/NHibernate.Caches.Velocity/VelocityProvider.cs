...
namespace NHibernate.Caches.Velocity
{
    ...
    public class VelocityProvider : ICacheProvider
	{
		...
		{
            _log = LogManager.GetLogger(typeof(VelocityProvider));
            VelocityConfig[] configs = ConfigurationManager.GetSection("velocity") as VelocityConfig[];
			if (configs != null)
			{
                foreach (VelocityConfig config in configs)
				{
				}
			}
		}
