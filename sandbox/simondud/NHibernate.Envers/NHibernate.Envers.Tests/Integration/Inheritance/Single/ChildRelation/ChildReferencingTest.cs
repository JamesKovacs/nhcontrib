...
namespace NHibernate.Envers.Tests.Integration.Inheritance.Single.ChildRelation
{
	...
	public class ChildReferencingTest : TestBase
	{
		...
		{
			CollectionAssert.AreEquivalent(new[] { 1, 2, 3 }, AuditReader.GetRevisions(typeof(ReferencedEntity), re_id1));
			CollectionAssert.AreEquivalent(new[] { 1, 3 }, AuditReader.GetRevisions(typeof(ReferencedEntity), re_id2));
		...
		{
			CollectionAssert.IsEmpty(AuditReader.Find<ReferencedEntity>(re_id2, 1).Referencing);
			CollectionAssert.IsEmpty(AuditReader.Find<ReferencedEntity>(re_id2, 2).Referencing);
			CollectionAssert.AreEquivalent(new[] { new ChildIngEntity { Id = c_id, Data = "y", Number = 1 } }, AuditReader.Find<ReferencedEntity>(re_id2, 3).Referencing);
		}
		...
		public void VerifyChildHistory()
		{
			...
			Assert.AreEqual(new ReferencedEntity { Id = re_id1 }, AuditReader.Find<ChildIngEntity>(c_id, 2).Referenced);
			Assert.AreEqual(new ReferencedEntity { Id = re_id2 }, AuditReader.Find<ChildIngEntity>(c_id, 3).Referenced);
		}
