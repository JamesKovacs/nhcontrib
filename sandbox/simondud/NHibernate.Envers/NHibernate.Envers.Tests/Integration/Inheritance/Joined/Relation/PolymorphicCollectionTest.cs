...
namespace NHibernate.Envers.Tests.Integration.Inheritance.Joined.Relation
{
	...
	public class PolymorphicCollectionTest : TestBase
	{
		...
		protected override void Initialize()
		{
			...
			p_id = 10;
			c_id = 100;
			var re = new ReferencedEntity {Id = ed_id1};
			var pie = new ParentIngEntity {Id = p_id, Data = "x", Referenced = re};
			var cie = new ChildNotIngEntity {Id = c_id, Data = "y", Number = 1l, Referenced = re};
		...
		{
			CollectionAssert.AreEquivalent(new[] { 1, 2, 3 }, AuditReader.GetRevisions(typeof(ReferencedEntity), ed_id1));
		...
		{
			CollectionAssert.IsEmpty(AuditReader.Find<ReferencedEntity>(ed_id1, 1).Referencing);
			CollectionAssert.AreEquivalent(new[] { new ParentIngEntity { Id = p_id, Data = "x" } },
										AuditReader.Find<ReferencedEntity>(ed_id1, 2).Referencing);
			CollectionAssert.AreEquivalent(new[] { new ParentIngEntity { Id = p_id, Data = "x" }, new ChildNotIngEntity{ Id = c_id, Data = "y", Number = 1} },
										AuditReader.Find<ReferencedEntity>(ed_id1, 3).Referencing);
		}
